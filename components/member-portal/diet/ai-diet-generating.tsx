  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 space-y-8">
      ...
      <div className="relative">
        <div className="w-24 h-24 rounded-full border-4 border-slate-700 border-t-cyan-500 animate-spin" />
        <div className="absolute inset-0 flex items-center justify-center">
          <CurrentIcon className="h-10 w-10 text-cyan-400 animate-pulse" />
        </div>
      </div>
        ...
        {MESSAGES.map((_, i) => (
          <div
            ...
            className={`w-2 h-2 rounded-full transition-all duration-300 ${
              i <= messageIndex ? 'bg-cyan-500' : 'bg-slate-700'
            }`}
          />
        ))}
      </div>

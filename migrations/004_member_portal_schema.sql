-- ============================================================
-- Migration: Member Portal - Workout Tracking & Diet Plans
-- Created: 2026-02-15
-- Description: Adds member portal features including workout
--              routines, sessions, diet plans, and magic link auth
-- ============================================================
...
-- Create unique index to ensure one meal per type per day
-- Using cast to date (immutable) instead of DATE() function
CREATE UNIQUE INDEX IF NOT EXISTS idx_meal_plans_unique_per_day
ON public.meal_plans(diet_plan_id, (scheduled_for::date), meal_type);

CREATE INDEX IF NOT EXISTS idx_meal_plans_diet_plan_date
ON public.meal_plans(diet_plan_id, scheduled_for DESC);
...
-- Note: This will insert system-wide exercises for ALL gyms
-- Each gym gets its own copy of these exercises (isolated by gym_id)

-- Function to seed exercises for a specific gym
...
BEGIN
    RAISE NOTICE '========================================'    ;
    RAISE NOTICE 'Member Portal Migration Completed!'    ;
    RAISE NOTICE '========================================'    ;
